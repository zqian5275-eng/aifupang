...
print("=== 个股数据 ===")
items = push2_get('m:0+t:6,m:1+t:2,m:0+t:13', 'f2,f3,f4,f5,f6,f12,f14,f20,f21,f23,f24,f25,f115,f184,f100,f102', pz=100, max_pages=60)
rows = []
for item in items:
    pe = item.get('f115','-')
...
print("=== 百日新高 ===")
items = push2_get('m:0+t:6,m:1+t:2', 'f2,f3,f12,f14,f6,f100', fid='f3', pz=200, max_pages=10)
rows = [{'代码': i.get('f12',''), '名称': i.get('f14',''), '涨幅': i.get('f3',0), '所属概念': i.get('f100',''), '上榜原因': '涨幅榜', '成交额': i.get('f6',0)} for i in items if float(i.get('f3',0) or 0) > 5]
